...
def make_selection_fn(mode):
    if mode == "no_quality_and_syntax":

        def fn(candidates, wm_state, wm_thr):
            def select(candidate_indices):
                if not candidate_indices:
                    return None, False
                return random.choice(candidate_indices), True

            wm_ok_semanteme = [
                i for i, c in enumerate(candidates) if (c["HMAC_semanteme"] % 2 == 1)
            ]
            if wm_ok_semanteme:
                return select(wm_ok_semanteme)[0]

            all_indices = list(range(len(candidates)))
            return select(all_indices)[0]
        ...
        def fn(candidates, wm_state, wm_thr):
            def select(candidate_indices):
                ...
                sorted_by_score = sorted(
                    candidate_indices, key=get_norm_score, reverse=True
                )
                return sorted_by_score[0], True

            wm_ok_semanteme = [
                i for i, c in enumerate(candidates) if (c["HMAC_semanteme"] % 2 == 1)
            ]
            if wm_ok_semanteme:
                chosen_idx, _ = select(wm_ok_semanteme)
                return chosen_idx

            all_indices = list(range(len(candidates)))
            chosen_idx, _ = select(all_indices)
            return chosen_idx
        ...
        def fn(candidates, wm_state, wm_thr):
            ...
            wm_ok_syntax = [
                i for i, c in enumerate(candidates) if (c["HMAC_syntax"] % 2 == 1)
            ]
            if wm_ok_syntax:
                return select(wm_ok_syntax)[0]
...
def generate(prompt, args, model=None, device=None, tokenizer=None):
    ...
    default_selection_fn = make_selection_fn(args.ablation_mode)
        ...
        try:
            for step in range(max_new_tokens):
                ...
                topk_scores, topk_indices = torch.topk(
                    candidate_scores_flat, k=beam_width * 2
                )
    ...
    decoded_output_with_watermark = tokenizer.decode(
        final_output_ids[0], skip_special_tokens=True
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="IPMark ablation study")
    parser.add_argument("--model_name_or_path", type=str, default="facebook/opt-1.3b")
    parser.add_argument("--ablation_mode", type=str, default="no_quality",
                        choices=["no_quality", "no_syntax", "no_quality_and_syntax"],
    ...
    parser.add_argument("--save_path", type=str, default="output_ablation.jsonl")
